...
The entire sequence is coordinated by the `runPipeline(jobId, userId, request)` function inside the orchestrator library. It runs as a detached promise in the Next.js server process and transitions the job through six distinct sequential stages. Progress is written to the job's row in Supabase Postgres after every step.

Renders need real files on a real disk, so each job gets a scratch directory at `{MANIMATE_WORK_DIR}/{jobId}` — referenced below as `{work}`. It is deleted when the pipeline finishes; only the final video and the generated Python are uploaded to Supabase Storage.
...
- **Implementation**: [fetchWebResearch()](file://./src/lib/manimate/webResearch.ts#L3-L29)
- **Workflow**:
...
- **Implementation**: [generateLecturePlan()](file://./src/lib/manimate/llm.ts#L133-L150)
- **Workflow**:
  1. Resolves the selected LLM provider and model.
  2. Prompts the LLM with `PLANNER_PROMPT` (from [prompts.ts](file://./src/lib/manimate/prompts.ts)), passing the topic, desired topic depth (`brief`, `normal`, or `deep`), and the compiled Tavily web research context.
...
- **Implementation**: [generateManimForModule()](file://./src/lib/manimate/llm.ts#L152-L178)
- **Workflow**:
  ...
  5. Writes each scene's code to `{work}/scene_code/module_{moduleIndex}_{sceneId}.py`. `sceneId` originates in LLM JSON and is sanitised first, so it cannot escape the directory.
...
- **Implementation**: [renderSceneWithCorrections()](file://./src/lib/manimate/manim.ts#L53-L97)
- **Workflow**:
  1. Resolves the Python environment path via [defaultManimPython()](file://./src/lib/manimate/manim.ts#L41-L43) (defaults to `manim-env/Scripts/python.exe` in the project root, or `MANIM_PYTHON` environment variable).
  2. Invokes Manim using the command line:
     ```bash
     python -m manim $MANIM_QUALITY --media_dir <mediaDir> <pyFile> <className>
     ```
     `MANIM_QUALITY` defaults to `-qm` (720p30). `-qh` (1080p60) renders ~4x slower and produces files past Supabase Storage's default 50 MB per-object limit. Each correction attempt renders into its own `attempt_N/` media directory, so `newestMp4()` cannot pick up a previous attempt's partial movie files. The child process is spawned with a minimal environment, so generated Python never sees the server's API keys.
  ...
  4. If compilation fails (throws stderr/tracebacks):
     ...
     - **Compiler Correction Loop**: If attempts remain (`max_correction_attempts`, default `3`), invokes [correctManimCode()](file://./src/lib/manimate/llm.ts#L179-L193).
...
- **Implementation**: [generateTtsAudio()](file://./src/lib/manimate/tts.ts#L21-L35)
- **Workflow**:
  ...
  4. Synthesizes the scene voiceover script using the chosen voice (default: `'af_heart'`) and saves it to `{work}/tts/module_{moduleIndex}_{sceneId}.wav`.
  5. Muxes the audio track into the scene video using [muxVoiceover()](file://./src/lib/manimate/video.ts#L38-L78).
...
- **Implementation**: [stitchFinal()](file://./src/lib/manimate/video.ts#L80-L89)
- **Workflow**:
  ...
  2. For each module, writes a list file containing video paths and runs the FFmpeg concat demuxer:
     ...
     ```
  3. Once module videos are completed, runs the concat demuxer again to join the modules into `{work}/video.mp4`.
  4. Uploads that file to Supabase Storage at `{user_id}/{job_id}/video.mp4` (resumably, since it exceeds 6 MB), then uploads the generated `.py` files alongside it.
  5. Marks the row `completed` and records the Storage object key in `final_video_path`.
  6. Deletes the scratch directory.
